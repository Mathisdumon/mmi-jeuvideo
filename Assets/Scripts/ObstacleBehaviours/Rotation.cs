...
public class Rotation : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		{ //Si le timer de l'angle vaut 0 mais que l'objet est en pause (pause non égal à 0)
			if (!angleRectify)
			{ //Si c'est la première fois qu'il est en pause, on réajuste l'angle de l'objet à sa position d'angle exacte pour éviter des micro-décalages qui s'accumulent
			...
			if (timer > 0)
			{ //Si le timer est supérieur à 0, on enlève le laps de temps écoulé Time.fixedDeltaTime au timer (sans dépasser 0, grâce à la fonction max)
				timer = Mathf.Max(0, timer - Time.fixedDeltaTime);
			}
			else
			{ //Si le timer est égal à 0, on réinitialise timerAngle à la valeur de angle et timer à la valeur de pause, on remet angleRectify à false
				timer = pause;
				timerAngle = angle;
				angleRectify = false;
			}
		...
		{ //Si l'objet n'est pas en pause, on le fait tourner via son transform. On soustrait l'angle parcouru en une frame au timerAngle en valeur absolu pour ne pas dépendre du sens de rotation (sans dépasser 0, grâce à la fonction max)
