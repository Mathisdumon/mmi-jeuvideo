...
public class TransparentFlashing : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		else if (_timer > 0)
		{
			...
			if (_step == 1)
			{ //Si on est sur l'étape 1, transition d'opaque à transparent, la transparence du sprite est un nombre décimal égal au ratio de la valeur du timer / la valeur de durée de la transition
				_sprite.color = new Color(_sprite.color.r, _sprite.color.g, _sprite.color.b, _timer / goToTransparent);
			}
			else if (_step == 3)
			{ //Si on est sur l'étape 3, transition de transparent à opaque, la transparence du sprite est un nombre décimal égal à 1 - le ratio de la valeur du timer / la valeur de durée de la transition
				_sprite.color = new Color(_sprite.color.r, _sprite.color.g, _sprite.color.b, 1 - (_timer / goToOpaque));
			}
		}
		else
		{ // Si le timer vaut 0 (a fini de s'écouler), on passe à l'étape suivante (revient à 0 si on dépasse 4)
			_step = (_step + 1) % 4;
