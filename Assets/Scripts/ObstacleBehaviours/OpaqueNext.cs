...
public class OpaqueNext : MonoBehaviour
{
	...
	void Update()
	{
		...
		{ //Si le joueur est assez proche de l'objet
			if (distanceFromPlayer > distance - marge)
			{ //Si le joueur est entre la distance max et la marge
				if (isFullOpaque)
				{ //On met à faux la variable d'état pleinement opaque
					isFullOpaque = false;
				}
				if (isFullTransparent)
				{ //On met à faux la variable d'état pleinement transparent
